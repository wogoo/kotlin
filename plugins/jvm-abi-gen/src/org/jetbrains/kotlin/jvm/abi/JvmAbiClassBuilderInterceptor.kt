 ...
 * remove private functions, and copy inline functions verbatim. There are
 * two corner cases to consider.
 *
 * First, if we have an inline function which is not callable from Java (i.e.,
 * due to reified type parameters), we generate a private method, which nevertheless
 * has to be copied verbatim.
 *
 * Second, for a inline suspend function `f` we will usually generate two methods,
 * a *non-inline* method `f` and an inline method `f$$forInline`. The former can
 * be stripped. However, if `f` is not callable directly, we only generate a single
 * *inline* method `f`.
 */
