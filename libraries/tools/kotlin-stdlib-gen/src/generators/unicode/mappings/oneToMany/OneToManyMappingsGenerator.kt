...
internal class OneToManyMappingsGenerator private constructor(
    ...
    companion object {
        fun forUppercase(outputFile: File, target: KotlinTarget, unicodeDataLines: List<UnicodeDataLine>): OneToManyMappingsGenerator {
            val builder = OneToManyUppercaseMappingsBuilder(unicodeDataLines)
        ...
        fun forLowercase(outputFile: File, target: KotlinTarget, unicodeDataLines: List<UnicodeDataLine>): OneToManyMappingsGenerator {
            val builder = OneToManyLowercaseMappingsBuilder(unicodeDataLines)
            val writer = OneToManyLowercaseMappingsWriter(RangesWritingStrategy.of(target, "OneToManyLowercase"))
            return OneToManyMappingsGenerator(outputFile, builder, writer)
        }

        fun forTitlecase(outputFile: File, target: KotlinTarget, unicodeDataLines: List<UnicodeDataLine>): OneToManyMappingsGenerator {
            val builder = OneToManyTitlecaseMappingsBuilder(unicodeDataLines)
            val writer = OneToManyTitlecaseMappingsWriter(RangesWritingStrategy.of(target, "OneToManyTitlecase"))
            return OneToManyMappingsGenerator(outputFile, builder, writer)
        }
    }
}
