...
import org.jetbrains.kotlin.gradle.targets.native.internal.CInteropCommonizerTarget
import org.jetbrains.kotlin.gradle.targets.native.internal.contains
import org.jetbrains.kotlin.gradle.targets.native.internal.getCInteropCommonizerTarget
import org.jetbrains.kotlin.konan.target.KonanTarget.*
import kotlin.test.*

class CInteropCommonizerTargetTest {
    ...
    fun `nativeMain linux macos`() {
        ...
        assertEquals(
            CInteropCommonizerTarget(CommonizerTarget(LINUX_X64, MACOS_X64), setOf(linuxInterop.identifier, macosInterop.identifier)),
            project.getCInteropCommonizerTarget(nativeMainCompilation)
        )
    }
    ...
    fun `nativeMain iosMain linux macos iosX64 iosArm64`() {
        ...
        assertEquals(
            ...
            project.getCInteropCommonizerTarget(sharedNativeCompilation(nativeMain))
        )

        assertEquals(
            CInteropCommonizerTarget(CommonizerTarget(IOS_X64, IOS_ARM64), setOf(iosX64Interop, iosArm64Interop)),
            project.getCInteropCommonizerTarget(sharedNativeCompilation(iosMain))
        )

        assertTrue(
            project.getCInteropCommonizerTarget(sharedNativeCompilation(iosMain))!! in
                    project.getCInteropCommonizerTarget(sharedNativeCompilation(nativeMain))!!,
            "Expected CInteropCommonizerTarget of iosMain to be fully contained in nativeMain"
        )

        assertFalse(
            project.getCInteropCommonizerTarget(sharedNativeCompilation(nativeMain))!! in
                    project.getCInteropCommonizerTarget(sharedNativeCompilation(iosMain))!!,
            "Expected CInteropCommonizerTarget of nativeMain to not be contained in iosMain"
        )
