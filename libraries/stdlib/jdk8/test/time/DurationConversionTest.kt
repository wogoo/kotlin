...
class DurationConversionTest {
    ...
    fun twoWayConversion() {
        fun test(days: Int, hours: Int, minutes: Int, seconds: Int, millis: Int, nanos: Int) {
            val duration =
                days * Duration.DAY_24H + hours * Duration.HOUR + minutes * Duration.MINUTE + seconds * Duration.SECOND +
                        millis * Duration.MILLISECOND + nanos * Duration.NANOSECOND
    ...
    fun javaToKotlinRounding() {
        ...
        assertEquals(105 * Duration.DAY_24H, duration)
    }

    @Test
    fun kotlinToJavaClamping() {
        val duration = (Long.MAX_VALUE * Duration.SECOND) * 5
    ...
    fun randomIsoConversionEquivalence() {
        repeat(100) {
            val duration = Random.nextLong(-(1 shl 53) + 1, 1 shl 53) * Duration.NANOSECOND
